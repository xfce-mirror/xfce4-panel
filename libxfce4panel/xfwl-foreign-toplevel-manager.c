 ...
 *
 * Wrapping in a GObject allows to allocate only one toplevel manager and to cache
 * its resources only once, offering at least the usual getters to access them.
 **/
 ...
 *
 * An opaque structure wrapping a `struct zwlr_foreign_toplevel_manager_v1`.
 **/
...
{
  /* triggers finalize() */
  g_object_remove_toggle_ref (data, xfwl_foreign_toplevel_manager_stop, wl_manager);
}
 ...
 * the existing one, if the `wlr_foreign_toplevel_manager_v1` protocol is supported.
 * %NULL otherwise. To be released with g_object_unref() when no longer used.
 **/
XfwlForeignToplevelManager *
 ...
 * @manager: an #XfwlForeignToplevelManager
 *
 * Returns: (transfer none): The underlying Wayland object.
 **/
