...
static void
wrapper_plug_wayland_add (GtkContainer *plug,
                          GtkWidget *provider,
                          GDBusProxy *proxy)
{
  g_signal_handlers_disconnect_by_func (plug, wrapper_plug_wayland_add, proxy);
  wrapper_plug_proxy_provider_signal (XFCE_PANEL_PLUGIN_PROVIDER (provider),
                                      PROVIDER_SIGNAL_EMBEDDED, proxy);
}
...
GtkWidget *wrapper_plug_wayland_new (GDBusProxy *proxy)
{
  ...
  g_signal_connect (plug, "add", G_CALLBACK (wrapper_plug_wayland_add), proxy);
