                                             ...
                                             PanelWindow *window)
{
  ...
  if (window->autohide_behavior == AUTOHIDE_BEHAVIOR_INTELLIGENTLY
      && window->autohide_block == 0)
    {
      ...
      else if (! GDK_IS_X11_DISPLAY (window->display) || active_window == NULL)
        return;
                                     ...
                                     PanelWindow *window)
{
  ...
  toplevels = xfwl_foreign_toplevel_manager_get_toplevels (window->wl_toplevel_manager);
  for (lp = toplevels; lp != NULL; lp = lp->next)
    if (lp->data != toplevel
        && xfwl_foreign_toplevel_get_state (lp->data) & XFWL_FOREIGN_TOPLEVEL_STATE_MAXIMIZED
        && panel_window_toplevel_on_panel_monitor (window, lp->data))
      {
        g_signal_handlers_disconnect_by_func (lp->data, panel_window_toplevel_active_closed, window);
