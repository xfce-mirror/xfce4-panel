...
/* settings */
static void pager_set_size(Control *control, int size)
{
    t_pager *pager;
...
static void pager_set_orientation(Control *control, int orientation)
{
    t_pager *pager;
 ...
 *  ---------------------------------------
*/
static void pager_attach_callback(Control *control, const char *signal, 
				     GCallback callback, gpointer data)
{
    SignalCallback *sc;
    t_pager *pager;
...
static void arrange_pager(t_pager *pager)
{
    ...
    gtk_box_pack_start(GTK_BOX(pager->box), pager->separators[0], TRUE, TRUE, 2);
	...
	gtk_box_pack_start(GTK_BOX(pager->box), align, 
		    	       TRUE, TRUE, 2);
    }
    
    gtk_box_pack_start(GTK_BOX(pager->box), pager->separators[1], TRUE, TRUE, 2);
...
/* callbacks */
static void pager_screen_created(NetkScreen *screen, NetkWorkspace *ws,
				    t_pager *pager)
{
    netk_pager_update_size(pager->netk_pager, pager->screen);
}

static void pager_screen_destroyed(NetkScreen *screen, NetkWorkspace *ws,
				      t_pager *pager)
{
    netk_pager_update_size(pager->netk_pager, pager->screen);
}

t_pager *pager_new(NetkScreen *screen)
{
...
static void pager_free(Control *control)
{
    GList *li;
    t_pager *pager;
 ...
 *  ----------------------
*/
gboolean create_pager_control(Control *control)
{
    t_pager *pager;
    NetkScreen *screen;
