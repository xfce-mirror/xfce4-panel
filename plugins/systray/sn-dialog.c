  ...
  if (gtk_builder_add_from_resource (dialog->builder, "/org/xfce/panel/sn-dialog.glade", &error))
    {
      ...
      object = gtk_builder_get_object (dialog->builder, "checkbutton-temp-reveal-attention-items");
      g_return_val_if_fail (GTK_IS_WIDGET (object), FALSE);
      g_object_bind_property (G_OBJECT (dialog->config), "temp-reveal-attention-items",
                              G_OBJECT (object), "active",
                              G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
