...
{
  panel_return_if_fail (GTK_IS_DIALOG (dialog), NULL);

  PanelUtilsGtkLabelData *label_data;
  GtkLabel *label;
  label_data = g_slice_new0 (PanelUtilsGtkLabelData);
  label_data->label_text = g_strdup (label_text);
  ...
  g_free (label_data->label_text);
  g_slice_free (PanelUtilsGtkLabelData, label_data);
  return label;
}
